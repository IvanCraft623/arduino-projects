...
// *********************************************************************
  ...
  unsigned long g_LCDML_DISP_press_time = 0;
    // settings
    #define _LCDML_CONTROL_analog_pinx A2
    #define _LCDML_CONTROL_analog_piny A3
    #define _LCDML_CONTROL_digitalread 0

    // when you did not use a button set the value to zero
    #define _LCDML_CONTROL_analog_up_min 0 // Button Up
    #define _LCDML_CONTROL_analog_up_max 412
    #define _LCDML_CONTROL_analog_down_min 612 // Button Down
    #define _LCDML_CONTROL_analog_down_max 1023
...
void lcdml_menu_control(void)
{
  ...
  if(LCDML.BT_setup()) {
    ...
    digitalWrite (_LCDML_CONTROL_digitalread, HIGH);
  }
  // check debounce timer
  if((millis() - g_LCDML_DISP_press_time) >= 200) {
    g_LCDML_DISP_press_time = millis(); // reset debounce timer
